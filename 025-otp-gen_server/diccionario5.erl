...
%% API
-export([start_link/0, store/2, get/1, get_keys/0, clear/0, sleep/0, sleep2/0, stop/0]).
...
sleep2() ->
    gen_server:call(?MODULE, {sleep}, 8000).
...
handle_call({sleep}, _From, State) ->
    timer:sleep(6000),
    {reply, ok, State};
handle_call(Mensaje, _From, State) ->
    io:format("Se recibio un mensaje sincronico no esperado ~p~n", [Mensaje]),
    {noreply, State}.
...
terminate(_Reason, _State) ->
    io:format("Terminando el proceso, haciendo limpieza~n"),
    ok.
